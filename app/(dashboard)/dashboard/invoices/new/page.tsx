...
export default function NewInvoicePage() {
  ...
  const handleSubmit = async (status: "DRAFT" | "SENT") => {
    ...
    try {
      ...
      const lastRes = await fetch("/api/invoices?limit=1");
      const lastData = await lastRes.json();
      const lastInvoice = lastData.data?.[0];
      const lastNum = lastInvoice?.invoiceNumber?.match(/\d+$/)?.[0];
      ...
      const res = await fetch("/api/invoices", {
        ...
        body: JSON.stringify({
          invoiceNumber,
          clientId, issueDate, dueDate, status,
          items, discountType, discountValue,
          isGst, gstType, notes, terms,
        }),
      });
      ...
      toast.success(
        status === "SENT" ? "Invoice sent!" : "Invoice saved as draft"
      );
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-white/40 text-sm mt-0.5">Create a new invoice</p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={() => handleSubmit("SENT")} disabled={isSaving}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <Card className="border border-white/5 bg-white/[0.02] rounded-2xl">
              <CardHeader>
                <CardTitle className="text-white text-base">Invoice Details</CardTitle>
              </CardHeader>
                ...
                {items.map((item, i) => (
                  <div key={i}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      ...
                      <div className="space-y-1.5">
                        <Label className="text-white/60 text-xs">Rate (₹)</Label>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Select value={String(item.taxRate)}
                          onValueChange={(v) => updateItem(i, "taxRate", parseInt(v))}>
                          <SelectTrigger className="bg-white/5 border-white/10 text-white focus:border-[#1a472a] rounded-lg h-9 text-sm">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent className="bg-[#0f1f14] border-white/10">
                    ...
                    {/* HSN Code */}
                    <div className="space-y-1.5">
                      <Label className="text-white/60 text-xs">HSN/SAC Code</Label>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <Card className="border border-white/5 bg-white/[0.02] rounded-2xl">
              <CardHeader>
                <CardTitle className="text-white text-base">Notes & Terms</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  <Label className="text-white/60 text-sm">Terms & Conditions</Label>
        ...
        <div className="space-y-5">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <Card className="border border-white/5 bg-white/[0.02] rounded-2xl">
              <CardHeader>
                <CardTitle className="text-white text-base">GST Settings</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
              ...
              <CardContent className="space-y-3">
                <div className="flex gap-2">
                  <Button size="sm" variant="outline"
                    ...
                    className={`flex-1 rounded-lg text-xs ${discountType === "PERCENTAGE"
                  ...
                  <Button size="sm" variant="outline"
                    ...
                    className={`flex-1 rounded-lg text-xs ${discountType === "FIXED"
                      ? "bg-[#1a472a] border-[#1a472a] text-white"
                      : "border-white/10 bg-white/5 text-white/60"}`}>
                    ₹ Fixed
                  </Button>
            ...
            <Card className="border border-white/5 bg-white/[0.02] rounded-2xl sticky top-6">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="pt-2 space-y-2">
                  <Button className="w-full bg-[#1a472a] hover:bg-[#1a472a]/80 text-white gap-2"
                    onClick={() => handleSubmit("SENT")} disabled={isSaving}>
                    <Send className="w-4 h-4" />
                    {isSaving ? "Saving..." : "Save & Send"}
                  </Button>
                  <Button variant="outline"
                    className="w-full border-white/10 bg-white/5 hover:bg-white/10 text-white gap-2"
                    onClick={() => handleSubmit("DRAFT")} disabled={isSaving}>
