...
function AnimatedGlobe() {
  ...
  useEffect(() => {
    ...
    function drawGlobe() {
      ...
      glow.addColorStop(0, "rgba(34,197,94,0.08)"); glow.addColorStop(0.5, "rgba(34,197,94,0.04)"); glow.addColorStop(1, "rgba(34,197,94,0)");
      ctx.beginPath(); ctx.arc(cx, cy, r * 1.4, 0, Math.PI * 2); ctx.fillStyle = glow; ctx.fill();
      const grad = ctx.createRadialGradient(cx - r * 0.2, cy - r * 0.2, r * 0.1, cx, cy, r);
      grad.addColorStop(0, "rgba(22,101,52,0.9)"); grad.addColorStop(0.5, "rgba(15,68,35,0.85)"); grad.addColorStop(1, "rgba(5,30,15,0.95)");
      ...
      for (let lon = 0; lon < 180; lon += 20) {
        ...
        ctx.beginPath(); ctx.ellipse(cx, cy, r * Math.abs(Math.cos(a)), r, a > Math.PI / 2 && a < (3 * Math.PI) / 2 ? Math.PI / 2 : -Math.PI / 2, 0, Math.PI * 2); ctx.stroke();
      }
      ctx.restore();
      ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.strokeStyle = "rgba(34,197,94,0.5)"; ctx.lineWidth = 1.5; ctx.stroke();
      ctx.save(); ctx.translate(cx, cy); ctx.rotate(((angle * Math.PI) / 180) * 0.5); ctx.scale(1, 0.35);
      ctx.beginPath(); ctx.arc(0, 0, r * 1.3, 0, Math.PI * 2); ctx.strokeStyle = "rgba(34,197,94,0.25)"; ctx.lineWidth = 1; ctx.stroke();
      ctx.beginPath(); ctx.arc(r * 1.3, 0, 4, 0, Math.PI * 2); ctx.fillStyle = "rgba(34,197,94,0.9)"; ctx.fill(); ctx.restore();
      ctx.save(); ctx.translate(cx, cy); ctx.rotate(((-angle * Math.PI) / 180) * 0.7 + 1); ctx.scale(0.4, 1);
      ctx.beginPath(); ctx.arc(0, 0, r * 1.5, 0, Math.PI * 2); ctx.strokeStyle = "rgba(201,168,76,0.2)"; ctx.lineWidth = 1; ctx.stroke();
      ctx.beginPath(); ctx.arc(r * 1.5, 0, 3, 0, Math.PI * 2); ctx.fillStyle = "rgba(201,168,76,0.9)"; ctx.fill(); ctx.restore();
      ...
      angle += 0.3; animId = requestAnimationFrame(drawGlobe);
    }
    drawGlobe();
    return () => cancelAnimationFrame(animId);
  }, []);
  return <canvas ref={canvasRef} width={420} height={420} className="w-full max-w-[420px] mx-auto" style={{ filter: "drop-shadow(0 0 60px rgba(34,197,94,0.2))" }} />;
}
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center pt-20 pb-16 px-6 text-center">
      <motion.div variants={stagger} initial="hidden" animate="visible" className="flex flex-col items-center">
        ...
        <motion.h1 variants={fadeUp} transition={{ duration: 0.6 }} className="text-5xl md:text-7xl font-bold leading-tight mb-6 max-w-4xl">
          <span className="text-white">Get Paid Faster,</span><br />
          <span style={{ background: "linear-gradient(135deg, #22c55e, #c9a84c)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Invoice Smarter</span>
        </motion.h1>
        <motion.p variants={fadeUp} transition={{ duration: 0.6 }} className="text-white/50 text-lg md:text-xl max-w-xl mb-10 leading-relaxed">
        ...
        <motion.div variants={fadeUp} transition={{ duration: 0.6 }} className="flex flex-wrap justify-center gap-8 md:gap-16 pt-8 border-t border-white/10 w-full max-w-2xl">
          {[{ num: "50K+", label: "Freelancers" }, { num: "₹2.4Cr+", label: "Invoices Sent" }, { num: "4.9★", label: "Rating" }].map((s) => (
    ...
    <section id="features" className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
        <motion.div ref={ref} variants={stagger} initial="hidden" animate={inView ? "visible" : "hidden"} className="text-center mb-16">
          ...
          <motion.p variants={fadeUp} className="text-white/40 text-lg max-w-xl mx-auto">Built specifically for Indian freelancers, consultants, and agencies.</motion.p>
        </motion.div>
        <motion.div variants={staggerFast} initial="hidden" animate={inView ? "visible" : "hidden"} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((f, i) => (
            <motion.div key={i} variants={fadeUp} transition={{ duration: 0.5 }} whileHover={{ y: -4, transition: { duration: 0.2 } }}>
  ...
  return (
    <section id="how-it-works" className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div variants={stagger} initial="hidden" animate={inView ? "visible" : "hidden"} className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((s, i) => (
            <motion.div key={i} variants={fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }} className="text-center">
...
const plans = [
  { name: "Free", price: "₹0", period: "/forever", desc: "Perfect to get started", features: ["5 invoices/month", "1 client", "PDF download", "Basic GST"], cta: "Get Started", href: "/register", highlight: false },
  { name: "Pro", price: "₹299", period: "/month", desc: "For active freelancers", features: ["Unlimited invoices", "Unlimited clients", "UPI QR codes", "WhatsApp sharing", "Payment reminders", "Expense tracking"], cta: "Start Pro", href: "/register?plan=pro", highlight: true },
  { name: "Agency", price: "₹799", period: "/month", desc: "For teams & agencies", features: ["Everything in Pro", "5 team members", "Multiple businesses", "Priority support", "Custom branding", "API access"], cta: "Start Agency", href: "/register?plan=agency", highlight: false },
];
  ...
  return (
    <section id="pricing" className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div variants={stagger} initial="hidden" animate={inView ? "visible" : "hidden"} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan, i) => (
            <motion.div key={plan.name} variants={fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }} whileHover={{ y: plan.highlight ? -8 : -4, transition: { duration: 0.2 } }}>
                  ...
                  <ul className="space-y-3 mb-8">
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-center gap-2.5 text-sm text-white/60">
                        <Check className="w-4 h-4 text-[#22c55e] flex-shrink-0" />{f}
                      </li>
                    ))}
                  </ul>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div variants={stagger} initial="hidden" animate={inView ? "visible" : "hidden"} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <motion.div key={i} variants={fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }} whileHover={{ y: -4, transition: { duration: 0.2 } }}>
              <Card className="h-full p-6 rounded-2xl border border-white/5 bg-white/[0.02] hover:border-white/10 transition-colors">
                <CardContent className="p-0">
                  <div className="flex gap-1 mb-4">
                    {Array.from({ length: t.rating }).map((_, j) => <Star key={j} className="w-4 h-4 text-[#c9a84c] fill-[#c9a84c]" />)}
                  </div>
    ...
    <section id="faq" className="py-24 px-6">
      <div className="max-w-3xl mx-auto">
        ...
        <motion.div variants={stagger} initial="hidden" animate={inView ? "visible" : "hidden"} className="space-y-3">
          {faqs.map((faq, i) => (
            <motion.div key={i} variants={fadeUp} transition={{ duration: 0.4, delay: i * 0.06 }} className="border border-white/5 rounded-xl overflow-hidden">
  ...
  return (
    <section className="py-24 px-6">
      <div className="max-w-3xl mx-auto text-center">
        <motion.div ref={ref} variants={fadeUp} initial="hidden" animate={inView ? "visible" : "hidden"} transition={{ duration: 0.6 }}
          ...
          <div className="relative z-10">
            <motion.h2 variants={fadeUp} className="text-4xl md:text-5xl font-bold text-white mb-4">Ready to get paid faster?</motion.h2>
            <motion.p variants={fadeUp} className="text-white/40 text-lg mb-8">Join 50,000+ freelancers. Start free, no credit card required.</motion.p>
